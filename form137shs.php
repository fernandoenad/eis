		...
		?>
			...
			<tr>
				<td>
					...
					<tr><td width="8%" align="right">REMARKS &nbsp;</td><td colspan="3" style="BORDER-BOTTOM: black solid 1px"> <input type="text" style="width: 550px; border: 0px; font-weight: bold !important; font-size: 10px" value="<?php echo ($dataEnrolInfo['enrol_status1']=="PROMOTED"?$dataEnrolInfo['enrol_status2']:"NONE");?>" placeholder=""></td></tr>
					</table>		
				</td>
			</tr>
			<tr>
		...
		<td colspan="2" align="left"><br>Certified by:<br>
			<table width="100%"  cellpadding="0" cellspacing="0">
				<tr>
					...
					<td align="center"><input type="text" style="width: 65px; border: 0px; font-weight: bold !important; font-size: 10px; text-align: left;" value="<?php echo ($dataForm137['enrol_status']=="GRADUATED"?date('m/d/Y',  strtotime($dataForm137['enrol_graddate'])):"____________");?>"><br>Date</td>
				</tr>
				<tr>
...
<table width="780">
	...
	<tr>
		...
		?>
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
				<?php
				$checkCore = dbquery("select * from prospectus where (pros_curr='".$current_pros."' and pros_part='1') order by pros_title asc");
				while($dataCore=dbarray($checkCore)){
					if(substr($dataCore['pros_track'],-2)==$pros){
					$checkGrade = dbquery("select * from grade inner join class on grade_class_no=class_no inner join prospectus on class_pros_no=pros_no where (grade_stud_no='".$_GET['grade_stud_no']."' and pros_no='".$dataCore['pros_no']."')");
					$dataGrade = dbarray($checkGrade);
					$countGrade = dbrows($checkGrade);
				?>
	...
	<tr>
		<td colspan="2"><b>OTHER SUBJECTS</b>
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
				<?php
				$checkCore = dbquery("select * from prospectus where pros_track='SHS GENERAL' and pros_part='1' and pros_desc like '%Immersion%' order by pros_title asc");
				while($dataCore=dbarray($checkCore)){
					$checkGrade = dbquery("select * from grade inner join class on grade_class_no=class_no inner join prospectus on class_pros_no=pros_no where (grade_stud_no='".$_GET['grade_stud_no']."' and pros_no='".$dataCore['pros_no']."')");
					$dataGrade = dbarray($checkGrade);
					$countGrade = dbrows($checkGrade);
