...
class PDF extends FPDF{
	...
	function dependentsTable($header,$checkDependents,$teachSaln_no)
	{
		...
		$this->SetFont('','',8);
		//$this->SetFillColor(255,0,0);
		$checkSALNdet = dbquery("select * from teachsaln where teachSaln_no='".$teachSaln_no."'");
...
$pdf->SetFont('','',10);
$checkSALN = dbquery("select * from teachsaln where teachSaln_no='".$_GET['teachSaln_no']."'");
...
$checkSALN = dbquery("select * from teachsaln inner join teacher on teachSaln_teach_no=teach_no where teachSaln_no='".$_GET['teachSaln_no']."'");
$dataSALN = dbarray($checkSALN);
$pdf->SetFont('','IB',8);
$pdf->Cell(30,3,"",0,0,'L');
...
$pdf->SetFont('','B',8);
$pdf->Cell(8,4,"",0,0,'L');
$checkPosition = dbquery("select * from teacherappointments inner join dropdowns on teacherappointments_position=field_name where (teacherappointments_teach_no='".$dataSALN['teach_no']."' and teacherappointments_active='1')");
...
$pdf->Cell(63,4,strtoupper($current_school_full),'B',1,'L');
...
$checkSpouse = dbquery("select * from teachercontacts where (teachCont_teach_no='".$dataSALN['teach_no']."' and teachCont_type='1')");
...
$checkDependents = dbquery("select * from teachercontacts where (teachCont_teach_no='".$dataSALN['teach_no']."' and teachCont_type='2' and 	teachCont_bdate>='".$dateLimit."')");
...
$checkRealProperties = dbquery("select * from teachsalndetails where (teachSalnDet_teachSaln_no='".$_GET['teachSaln_no']."' and teachSalnDet_type='1')");
$header = array('', '', '', '', '', '', '', '', '');
$pdf->SetFont('','',8);
...
$checkPersonalProperties = dbquery("select * from teachsalndetails where (teachSalnDet_teachSaln_no='".$_GET['teachSaln_no']."' and teachSalnDet_type='2')");
$header = array('', '', '');
$pdf->SetFont('','',8);
...
$checkLiabilities = dbquery("select * from teachsalndetails where (teachSalnDet_teachSaln_no='".$_GET['teachSaln_no']."' and teachSalnDet_type='3')");
$header = array('', '', '');
$pdf->SetFont('','',8);
...
$checkBIFC = dbquery("select * from teachsalndetails where (teachSalnDet_teachSaln_no='".$_GET['teachSaln_no']."' and teachSalnDet_type='4')");
$countBIFC = dbrows($checkBIFC);
$pdf->SetFont('','',7);
...
$checkRelatives = dbquery("select * from teachsalndetails where (teachSalnDet_teachSaln_no='".$_GET['teachSaln_no']."' and teachSalnDet_type='5')");
$countRelatives = dbrows($checkRelatives);
$pdf->SetFont('','I',8);
...
$checkTeacherID = dbquery("select * from teacherids where teacherids_teach_no='".$dataSALN['teach_no']."'");
$dataTeacherID = dbarray($checkTeacherID );
$pdf->SetFont('','',8);
